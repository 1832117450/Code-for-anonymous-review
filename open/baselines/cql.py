...
class TrainConfig:
    # Experiment
    device: str = "cuda"
    env: str = "hopper-medium-v2"  # D4RL environment name
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = int(5e3)  # How often (time steps) we evaluate
    n_episodes: int = 10  # How many episodes run during evaluation
    max_timesteps: int = int(1e6)  # Max time steps to run environment
    checkpoints_path: Optional[str] = None  # Save path
    save_dir: Optional[str] = None  # Optional CSV output directory
    load_model: str = ""  # Model load file name, "" doesn't load
    resume_state_path: Optional[str] = None  # Latest full training state for resume
    ...
    norm_reward: bool = False  # normalize sparse rewards by the accumulation length
...
def save_resume_state(path: str, trainer: Any, step: int) -> None:
...
def load_resume_state(path: str, trainer: Any, device: str) -> int:
    ...
    print(f"Resumed training state from {path} at step={step}", flush=True)
    return step
...
class ContinuousCQL:
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        ...
        saved_log_alpha = state_dict["sac_log_alpha"]
        if saved_log_alpha is not None and self.log_alpha is not None:
            if isinstance(saved_log_alpha, Scalar):
                self.log_alpha.load_state_dict(saved_log_alpha.state_dict())
            else:
                self.log_alpha.load_state_dict(saved_log_alpha)
            self.alpha_optimizer.load_state_dict(
                state_dict=state_dict["sac_log_alpha_optim"]
            )

        saved_log_alpha_prime = state_dict["cql_log_alpha"]
        if isinstance(saved_log_alpha_prime, Scalar):
            self.log_alpha_prime.load_state_dict(saved_log_alpha_prime.state_dict())
        else:
            self.log_alpha_prime.load_state_dict(saved_log_alpha_prime)
...
def train(config: TrainConfig):
    ...
    if config.save_dir is not None:
        ...
        csv_path = os.path.join(config.save_dir, f"seed{config.seed}.csv")
    ...
    start_t = 0
    if config.resume_state_path and os.path.exists(config.resume_state_path):
        start_t = load_resume_state(config.resume_state_path, trainer, config.device)
        actor = trainer.actor

    wandb_init(asdict(config))

    evaluations = []
    for t in range(start_t, int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            if csv_writer is not None:
                csv_writer.writerow([t + 1, normalized_eval_score])
                csv_fh.flush()
            print("---------------------------------------")
            print(
            ...
            if config.resume_state_path is not None:
                save_resume_state(config.resume_state_path, trainer, t + 1)
            wandb.log(
